...
export default function Form({ setChartData }) {
  ...
  return (
    <div className={Style["form"]}>
      <div className={Style["browse__card"]}>
        <button className={Style["browse__button"]} {...getRootProps()}>
          <input type="file" accept={".csv"} hidden {...getInputProps()} />
          ...
          <span className={Style["browse__desc"]}>
            Click to select a file, or drag and drop it here.
          </span>
          <span className={Style["supported__format"]}>
            Supported Formates: CSV
          </span>
        </button>
        {selectedFile && (
          <section className={Style["selected__file"]}>
            <span>{selectedFile.name}</span>
            <button
              className={Style["delete__selected__file"]}
              onClick={removeFile}
            >
              <DeleteOutlineIcon sx={{ color: "#483EA8" }} />
            </button>
          </section>
        )}
      </div>
